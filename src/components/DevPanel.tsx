...
import type { DevEvent, DevLogEntry } from '../api/client'
...
function RequestEntry({ entry, isNew }: RequestEntryProps) {
...
function StateSection({ title, data }: StateSectionProps) {
...
export default function DevPanel() {
  ...
  const [requests, setRequests] = useState<DevLogEntry[]>([])
  ...
  useEffect(() => {
    function handleEvent({ type, entry }: DevEvent) {
      if (type === 'request') {
            ...
            requests.length === 0
              ...
              : requests.map(entry => (
                <RequestEntry
                  key={entry.id}
                  entry={entry}
                  isNew={entry.id !== undefined && newIds.has(entry.id)}
                />
              ))
          )}
          {activeTab === 'state' && (
