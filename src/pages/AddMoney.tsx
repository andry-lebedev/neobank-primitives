...
type Tab = 'SEPA' | 'SWIFT' | 'Stablecoin'

function SepaTab({ account }: { account?: Account }) {
...
function SwiftTab({ account }: { account?: Account }) {
...
function StablecoinTab({ wallet }: { wallet: Wallet | null }) {
  return (
    <div className="space-y-4">
      <Card className="p-5">
...
export default function AddMoney() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>('SEPA')
  ...
  const availableTabs = ([
    ...
    swiftAccount && 'SWIFT',
    'Stablecoin',
  ] as (Tab | undefined)[]).filter((tab): tab is Tab => Boolean(tab))
