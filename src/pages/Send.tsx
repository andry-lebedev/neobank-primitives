...
import type { Customer, Quote, Recipient, RecipientAccount, Transfer, Wallet } from '../types'
...
function BankPayoutFlow({ wallet, addTransfer, refreshWallet, kycOk, customer }: FlowProps) {
  ...
  function handleSelectRecipient(recipientId: string) {
...
function P2PFlow({ wallet, addTransfer, refreshWallet, kycOk }: Omit<FlowProps, 'customer'>) {
  const [step, setStep] = useState(1)
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-6 pb-28 space-y-5">
      ...
      <div className="flex gap-1 bg-card rounded-xl p-1">
        ...
        ] as { id: Mode; label: string; icon: LucideIcon }[]).map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            onClick={() => setMode(id)}
